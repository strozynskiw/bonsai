...
fn open_memory_edit_wizard(app: &mut AppState) {
    let Some(ModalKind::MemoryManager { rows, cursor }) = app.modal.as_ref() else {
        return;
    };
...
async fn submit_memory_add_wizard(app: &mut AppState, deps: RuntimeActionDeps<'_>) {
...
async fn toggle_selected_memory(app: &mut AppState, deps: RuntimeActionDeps<'_>) {
...
async fn delete_selected_memory(app: &mut AppState, deps: RuntimeActionDeps<'_>) {
    let Some((tier, name)) = selected_memory_identity(app) else {
        return;
    };
    let Some(memory) = deps.memory.clone() else {
