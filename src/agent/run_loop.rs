...
impl Agent {
    ...
    async fn lsp_diagnostic_baseline_for_batch(
        &self,
        batch: &[ToolCall],
    ...
    async fn resolve_rust_mutation_paths(&self, tool_call: &ToolCall) -> Vec<PathBuf> {
    ...
    fn project_relative_mutation_paths(&self, tool_call: &ToolCall) -> Vec<String> {
        mutation_paths(tool_call)
            .into_iter()
            .map(|raw| {
    ...
    async fn inject_new_lsp_diagnostics(
        &mut self,
